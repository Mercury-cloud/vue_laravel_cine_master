  ...
  <div>
    <div class="col-12 alert-notification" v-if="message === 'confirm_email'">
      <div class="offset-sm-1 offset-lg-1 email-confirmed">
        ...
        <p>Veuillez vérifier votre email, nous vous envoyons un code de confirmation.
          <br> Si vous n'avez pas reçu de mail de confirmation, cliquez sur
          <a v-if="show" @click="SEND_MAIL" style="color:#0275d8;">envoyer à nouveau</a>
          <a style="color:#0275d8;" v-else>Patientez..</a>
        </p>
      </div>
    </div>
      </div>
    ...
    <div class="col-12 alert-notification" v-if="message.cancel">
      <div class="offset-sm-1 offset-lg-1 email-confirmed">
        ...
        <path style="fill:#F2ECBF;" d="M51.695,37.69c-0.256,0-0.512-0.098-0.707-0.293c-0.391-0.391-0.391-1.023,0-1.414
          ...
          <div class="col-7 col-sm-10 col-lg-10 ">
            <h5>Votre abonnement se termine bientôt. </h5>
            <p class="text-muted">Votre dernier jour de visionnage sera le 
              <b style="color:#0275d8">{{message.cancel_time}}</b>, Si vous avez changé d'avis, reprenez votre abonnement dès maintenant.</p>
            <div class="btn-group-sm mt-1">
              <button class="btn btn-sm btn-warning ml-2 mt-1" @click="RESUME">RELANCER VOTRE ABONNEMENT</button>
              <button class="btn btn-sm btn-warning ml-2 mt-1" @click="message.cancel = null">ME RAPPELER PLUS TARD</button>
            </div>
          </div>
        </div>
      </div>
